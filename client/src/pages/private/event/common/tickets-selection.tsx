...
function TicketsSelection({ eventData }: { eventData: EventType }) {
  ...
  return (
    <div>
      <div>
        <h1 className="text-sm text-info font-bold">Selecciona el tipo de ticket</h1>

        <div className="flex flex-wrap gap-5 mt3">
          {ticketTypes.map((ticketType, index) => (
            <div key={index}
              className={`p-2 border border-gray-200 bg-gray-100 lg:w-96 w-full cursor-pointer ${selectedTicketType === ticketType.name ? 'bg-primary border-solid-2' : ''}`}
              onClick={() => {
            ...
            >
              ...
              <div className="flex justify-between">
                ...
                <h1 className="text-xs">{ticketType.limit} disponibles</h1>
              </div>
